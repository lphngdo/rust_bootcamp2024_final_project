...
pub trait ProvideEnergy<F: Fuel> {
    ...
    fn provide_energy_with_efficiency(&self, f: FuelContainer<F>, e: u8) -> <F as Fuel>::Output {
        let efficiency_value = e.min(EFFICIENCY_MAX);
        (f.amount * efficiency_value as u32 * F::energy_density().into() / PERCENTAGE_RATIO).into()
    }
...
impl<const DECAY: u32> InternalCombustion<DECAY> {
    pub fn new(efficiency: u8) -> Self {
        let efficiency_value = efficiency.min(EFFICIENCY_MAX);
        Self(RefCell::new((efficiency_value, 0)))
    }
}

impl<const DECAY: u32, F: Fuel> ProvideEnergy<F> for InternalCombustion<DECAY> {
    fn provide_energy(&self, f: FuelContainer<F>) -> <F as Fuel>::Output {
        let mut diasel = self.0.borrow_mut();
        diasel.0 = diasel.0.saturating_sub((diasel.1 / DECAY) as u8);
        diasel.1 += 1;
        self.provide_energy_with_efficiency(f, diasel.0)
    }
}
...
impl<const EFFICIENCY: u8, F: Fuel> ProvideEnergy<F> for OmniGenerator<EFFICIENCY> {
    fn provide_energy(&self, f: FuelContainer<F>) -> <F as Fuel>::Output {
        let efficiency_value = EFFICIENCY.min(EFFICIENCY_MAX);
        Self.provide_energy_with_efficiency(f, efficiency_value)
    }
}
